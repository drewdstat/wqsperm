...
`wqsperm` uses a `gwqs` object (from the `gWQS` package) as an input. To use `wqsperm`, we first need to run an initial *reference WQS regression* while setting `validation=0` and specifying a seed.  

Next, we need to create a second `gwqs` object for our *permutation test WQS regression* run with the **same random seed** and again setting `validation = 0`. However, for this second `gwqs` object, we may elect a bootstrap count lower than that in the reference object for the sake of efficiency. 

We will use this second `gwqs` object (*permutation test WQS regression*) as an input for the `wqsperm` package. We will put this object in the `model` `gwqs` argument and set the following parameters: 
...
# create permutation test wqs object with 200 bootstraps
wqs_perm <- gwqs(yLBX ~ wqs, mix_name = PCBs, data = wqs_data, q = 10, validation = 0, 
                b = 5, b1_pos = T, plan_strategy = "multicore", family = "gaussian", seed = 16)
