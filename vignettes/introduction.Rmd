...
$Y = \beta_0 + \beta_1(\sum_{i=1}^{c} w_i q_i) + z'\phi$ 

Where $Y$ is the outcome variable, $\beta_0$ is the intercept, $\beta_1$ is the coefficient
for the weighted quantile sum, $\sum_{i=1}^{c} w_i q_i$ is the weighted index for the set 
of quantiled mixture exposures, $z$ is the set of covariates, and $\phi$ is the regression
...
* `boots`: Number of bootstraps for the WQS run in each permutation test iteration. Note that we may elect a bootstrap count `boots` lower than that specified in the `model` object for the sake of efficiency. If we do, `wqs_perm` will generate a permutation test reference WQS run with that number of bootstraps to determine the reference $\beta_1$ value. 
...
wqs_main <- gwqs(yLBX ~ wqs, mix_name = PCBs, data = wqs_data, q = 10, validation = 0,
                b = 100, b1_pos = T, plan_strategy = "multicore", family = "gaussian", seed = 16)

# run permutation test
perm_test_res <- wqs_perm(wqs_main, niter = 100, boots = 50, b1_pos = T)
...
```{r wqs_full_perm}
perm_test_res <- wqs_full_perm(formula = yLBX ~ wqs,
                               data = wqs_data,
                               mix_name = PCBs,
                               q = 10,
                               b_main = 100,
