...
* `boots`: Number of bootstraps for the WQS run in each permutation test iteration. Note that we may elect a bootstrap count `boots` lower than that specified in the `model` object for the sake of efficiency. In we do, `wqs_perm` will generate a permutation test reference WQS run with that number of bootstraps to determine the reference beta_1 value. 
