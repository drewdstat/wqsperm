...
coefficient. The following permutation method gives us a p-value for the 
uncertainty while also allowing us to use the full dataset for training and 
validation. It works as follows:
...
* `gwqs_perm`: permutation test reference gWQS object (NULL if same number of 
bootstraps)
...
of bootstraps for the *permutation test reference WQS regression* run. As with 
before, it is acceptable/standard for `b_main` $>$ `b_perm` for the sake of 
efficiency. Finally, you should indicate the number of desired permutation test 
runs `niter`.
...
inputs where `family = gaussian` and `link = identity`. A logistic regression 
version for the algorithm is currently in the works. 
