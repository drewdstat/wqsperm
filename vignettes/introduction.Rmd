...
is described more in detail in (Day, 2022) and (Day, 2022).
...
`validation=0`. Note that permutation test can currently only take in `gwqs` 
...
permutation test WQS runs with the lower number of bootstraps. If `boots` is not 
specified, then the function will use the same bootstrap count in the 
...
* **perm_test**
  ...
  * *Logistic regression only*
    * **pval_se**: standard error for the p-value 
...
`stop_if_insig = TRUE` if you do not wish for the permutation test to proceed 
if the naive main WQS run produces an insignificant result (see 
...
perm_test_res1 <- wqs_perm(wqs_main1, niter = 10, boots = 5, b1_pos = T, seed = 16)
```
...
# run permutation test
perm_test_res2 <- wqs_perm(wqs_main2, niter = 10, boots = 5, b1_pos = T, seed = 16)
```
...
## Example 4 (using `wqsfullperm` on logistic regression example)
